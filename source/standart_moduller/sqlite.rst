...
Python'da bağlam yönetecisi oluşturmak için `with` deyimi kullanılır. Python `with`
deyimini kullanarak Sqlite ile işlem yaparken bir istisna ile karşılaşırsa yapılan
işlemlerin otomatik olarak geri alınmasını sağlar. Eğer işlem sorunsuz gerçekleşirse
herhangi bir etki yaratmaz. Aşağıda örnek koda bakalım::
    ...
    # Hata yakalanınca con.rollback() otomatik olarak çağırılır ve
    ...
    # Bağlam yöneticisi olarak kullanılan bağlantı nesnesi yalnızca işlemleri
    # gerçekleştirir veya geri alır, bu nedenle bağlantı nesnesi manuel olarak
    # kapatılmalıdır.
    con.close()
